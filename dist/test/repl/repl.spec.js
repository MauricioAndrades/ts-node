...
import { CMD_TS_NODE_WITHOUT_PROJECT_FLAG, delay, resetNodeEnvironment, ts, tsSupportsMtsCtsExtensions, } from '../helpers';
...
test.suite('top level await', ({ contextEach }) => {
    ...
    test.serial('should error with typing information when awaited result has type mismatch', async (t) => {
        ...
        expect(r.stderr.replace(/\r\n/g, '\n')).toBe(`${replFile}(4,7): error TS2322: ` +
...
test.suite('Multiline inputs and RECOVERY_CODES', (test) => {
    test.serial();
    test('multiline function args declaration', macroReplNoErrorsAndStdoutContains, `
      function myFn(
        a: string,
        b: string
      ) {
      ...
      myFn('test', '!')
    `, 'test !');
    test('Conditional recovery codes: this one-liner *should* raise an error; should not be recoverable', macroReplStderrContains, `
      (a: any) => a = null;
    `, 'error TS', {
...
test.suite('REPL treats object literals and block scopes correctly', (test) => {
    test('repl should treat { key: 123 } as object literal', macroReplNoErrorsAndStdoutContains, '{ key: 123 }', '{ key: 123 }');
    test('repl should treat ({ key: 123 }) as object literal', macroReplNoErrorsAndStdoutContains, '({ key: 123 })', '{ key: 123 }');
    test('repl should treat ({ let v = 0; v; }) as object literal and error', macroReplStderrContains, '({ let v = 0; v; })', 'No value exists in scope for the shorthand property');
    test('repl should treat { let v = 0; v; } as block scope', macroReplNoErrorsAndStdoutContains, '{ let v = 0; v; }', '0');
    test.suite('extra', (test) => {
        ...
        test('repl should treat { key: 123 }[] as block scope (edge case)', macroReplNoErrorsAndStdoutContains, '{ key: 123 }[]', '[]');
    });
    ...
    test.suite('property access', (test) => {
        test('repl should treat { key: 123 }.key as object literal property access', macroReplNoErrorsAndStdoutContains, '{ key: 123 }.key', '123');
        test('repl should treat { key: 123 }["key"] as object literal indexed access', macroReplNoErrorsAndStdoutContains, '{ key: 123 }["key"]', '123');
        test('repl should treat { key: 123 }.foo as object literal non-existent property access', macroReplStderrContains, '{ key: 123 }.foo', "Property 'foo' does not exist on type");
        test('repl should treat { key: 123 }["foo"] as object literal non-existent indexed access', macroReplStderrContains, '{ key: 123 }["foo"]', "Property 'foo' does not exist on type");
    });
});
