...
import { CMD_TS_NODE_WITHOUT_PROJECT_FLAG, createExec, ctxTsNode, testsDirRequire, TEST_DIR, tsSupportsImportAssertions, tsSupportsReact17JsxFactories, tsSupportsEs2022, } from './helpers';
...
test.suite('swc', (test) => {
    test('verify that TS->SWC target mappings suppport all possible values from both TS and SWC', async (t) => {
        const swcTranspiler = testsDirRequire('ts-node/transpilers/swc-experimental');
        // Detect when mapping is missing any ts.ScriptTargets
        const ts = testsDirRequire('typescript');
    ...
    `;
        ...
        test.suite('react 17 jsx factories', (test) => {
            test.if(tsSupportsReact17JsxFactories);
            test(compileMacro, { module: 'esnext', target: 'es2020', jsx: 'react-jsx' }, input, outdent `
          import { jsx as _jsx } from "react/jsx-runtime";
          const div = /*#__PURE__*/ _jsx("div", {});
        `);
            test(compileMacro, { module: 'esnext', target: 'es2020', jsx: 'react-jsxdev' }, input, outdent `
    ...
    test.suite('preserves import assertions for json imports', (test) => {
        ...
        test('basic json import', compileMacro, { module: 'esnext' }, outdent `
    ...
    test.suite('#1996 regression: ts-node gracefully allows swc to not return a sourcemap for type-only files', (test) => {
        ...
        test('use custom transpiler which never returns a sourcemap', async (t) => {
            const r = await exec(`${CMD_TS_NODE_WITHOUT_PROJECT_FLAG} --project tsconfig.custom-transpiler.json ./empty.ts`);
            expect(r.err).toBe(null);
