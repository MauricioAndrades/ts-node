...
import { BIN_PATH_JS, CMD_TS_NODE_WITH_PROJECT_TRANSPILE_ONLY_FLAG, tsSupportsMtsCtsExtensions, BIN_PATH, ROOT_DIR, TEST_DIR, testsDirRequire, ctxTsNode, CMD_TS_NODE_WITH_PROJECT_FLAG, CMD_TS_NODE_WITHOUT_PROJECT_FLAG, CMD_ESM_LOADER_WITHOUT_PROJECT, createExec, } from './helpers';
...
test.suite('ts-node', (test) => {
    ...
    test.suite('cli', (test) => {
        ...
        test('shows version of compiler via -vv', async () => {
            ...
            expect(r.err).toBe(null);
            expect(r.stdout.trim()).toBe(`ts-node v${testsDirRequire('ts-node/package').version}\n` +
        ...
        test('should print scripts', async () => {
            const r = await exec(`${CMD_TS_NODE_WITH_PROJECT_FLAG} -pe "import { example } from './complex/index';example()"`);
            expect(r.err).toBe(null);
        ...
        test.suite('should support cts when module = CommonJS', (test) => {
            ...
            test('test', async (t) => {
                const r = await exec([CMD_TS_NODE_WITHOUT_PROJECT_FLAG, '-pe "import { main } from \'./index.cjs\';main()"'].join(' '), {
        ...
        test('should eval code', async () => {
            const r = await exec(`${CMD_TS_NODE_WITH_PROJECT_TRANSPILE_ONLY_FLAG} -e "import * as m from './module';console.log(m.example('test'))"`);
            expect(r.err).toBe(null);
        ...
        test('should throw typechecking errors', async () => {
            const r = await exec(`${CMD_TS_NODE_WITH_PROJECT_FLAG} -e "import * as m from './module';console.log(m.example(123))"`);
            if (r.err === null) {
        ...
        test('should be able to ignore diagnostic', async () => {
            const r = await exec(`${CMD_TS_NODE_WITH_PROJECT_FLAG} --ignore-diagnostics 2345 -e "import * as m from './module';console.log(m.example(123))"`);
            if (r.err === null) {
                throw new Error('Command was expected to fail, but it succeeded.');
            }
        ...
        test('should support require flags', async () => {
            const r = await exec(`${CMD_TS_NODE_WITH_PROJECT_TRANSPILE_ONLY_FLAG} -r ./hello-world -pe "console.log('success')"`);
            expect(r.err).toBe(null);
            expect(r.stdout).toBe('Hello, world!\nsuccess\nundefined\n');
        });
        ...
        test('should support compiler scope specified via tsconfig.json', async (t) => {
            const r = await exec(`${CMD_TS_NODE_WITHOUT_PROJECT_FLAG} --project ./scope/c/config/tsconfig.json ./scope/c/index.js`);
            expect(r.err).toBe(null);
