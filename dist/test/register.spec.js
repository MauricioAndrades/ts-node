import 'lodash';
import { ctxTsNode, PROJECT_TRANSPILE_ONLY, resetNodeEnvironment, TEST_DIR } from './helpers';
...
test.suite('register(create(options))', (test) => {
    ...
    test('should use source maps', async () => {
        try {
            ...
            exp(error.stack).toMatch(['Error: this is a demo', `    at Foo.bar (${join(TEST_DIR, './throw error.ts')}:100:17)`].join('\n'));
        }
    });
