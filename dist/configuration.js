...
import { assign, attemptRequireWithV8CompileCache, createProjectLocalResolveHelper, getBasePathForProjectLocalDependencyResolution, } from './util';
/**
...
/** @internal */
export function findAndReadConfig(rawOptions) {
    const cwd = resolve(rawOptions.cwd ?? rawOptions.dir ?? DEFAULTS.cwd ?? process.cwd());
    const compilerName = rawOptions.compiler ?? DEFAULTS.compiler;
    // Compute minimum options to read the config file.
    let projectLocalResolveDir = getBasePathForProjectLocalDependencyResolution(undefined, rawOptions.projectSearchDir, rawOptions.project, cwd);
    ...
    const options = assign({}, DEFAULTS, tsNodeOptionsFromTsconfig || {}, { optionBasePaths }, rawOptions);
    ...
    if (configFilePath) {
        projectLocalResolveDir = getBasePathForProjectLocalDependencyResolution(configFilePath, rawOptions.projectSearchDir, rawOptions.project, cwd);
 ...
 * @internal
 */
export function readConfig(cwd, ts, rawApiOptions) {
    ...
    const { fileExists = ts.sys.fileExists, readFile = ts.sys.readFile, skipProject = DEFAULTS.skipProject, project = DEFAULTS.project, tsTrace = DEFAULTS.tsTrace, } = rawApiOptions;
 ...
 * @internal
 */
export function resolveAndLoadCompiler(name, relativeToPath) {
 ...
 * @internal
 */
export function getTsConfigDefaults(config, basePath, _files, _include, _exclude) {
    ...
    if (rootDir == null) {
        ...
        // Return this symbol to avoid computing from `files`, which would require fs calls
            ...
            rootDir = ComputeAsCommonRootOfFiles;
    }
