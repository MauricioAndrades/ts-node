...
import { parse as parseUrl, format as formatUrl, fileURLToPath, pathToFileURL } from 'url';
...
/** @internal */
export function filterHooksByAPIVersion(hooks) {
...
/** @internal */
export function registerAndCreateEsmHooks(opts) {
    // Automatically performs registration just like `-r ts-node/register`
    const tsNodeInstance = register(opts);
    return createEsmHooks(tsNodeInstance);
}
export function createEsmHooks(tsNodeService) {
    ...
    async function getFormat(url, context, defaultGetFormat) {
        ...
        if (!tsNodeService.ignored(nativePath) && (nodeSays.format === 'commonjs' || nodeSays.format === 'module')) {
            const { moduleType } = tsNodeService.moduleTypeClassifier.classifyModuleByModuleTypeOverrides(normalizeSlashes(nativePath));
            if (moduleType === 'cjs') {
                return { format: 'commonjs' };
            }
