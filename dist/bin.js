...
import { EVAL_FILENAME, EvalState, createRepl, setupContext, STDIN_FILENAME, EVAL_NAME, STDIN_NAME, REPL_FILENAME, } from './repl';
import { VERSION, TSError, register, createEsmHooks, createFromPreloadedConfig, DEFAULTS, } from './index';
 ...
 * @internal
 */
export function main(argv = process.argv.slice(2), entrypointArgs = {}) {
...
function phase4(payload) {
    ...
    if (isInChildProcess)
        require('./child/child-loader').lateBindHooks(createEsmHooks(service));
    ...
    if (showConfig) {
        ...
        const json = {
            ...
            ...ts.convertToTSConfig(service.config, service.configFilePath ?? join(cwd, 'ts-node-implicit-tsconfig.json'), service.ts.sys),
        };
        console.log(
