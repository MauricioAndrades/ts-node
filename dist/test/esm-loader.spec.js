...
import { BIN_ESM_PATH, BIN_PATH, BIN_PATH_JS, CMD_ESM_LOADER_WITHOUT_PROJECT, CMD_TS_NODE_WITHOUT_PROJECT_FLAG, ctxTsNode, nodeSupportsImportAssertions, nodeSupportsUnflaggedJsonImports, nodeUsesNewHooksApi, resetNodeEnvironment, TEST_DIR, tsSupportsImportAssertions, createExec, createSpawn, } from './helpers';
...
test.suite('esm', (test) => {
        ...
        function suite(name, tsconfig) {
            test.suite(name, (test) => {
                test('supports CJS webpack.config.ts in an otherwise ESM project', async (t) => {
                    ...
                    const r = await exec(`${CMD_TS_NODE_WITHOUT_PROJECT_FLAG} --project ./module-types/override-to-cjs/${tsconfig} ./module-types/override-to-cjs/test-webpack-config.cjs`);
                    expect(r.err).toBe(null);
                    expect(r.stdout).toBe(``);
                });
    ...
    test.suite('Entrypoint resolution falls back to CommonJS resolver and format', (test) => {
        test('extensionless entrypoint', async (t) => {
            const r = await exec(`${CMD_ESM_LOADER_WITHOUT_PROJECT} ./esm-loader-entrypoint-cjs-fallback/extensionless-entrypoint`);
            expect(r.err).toBe(null);
            expect(r.stdout.trim()).toBe('Hello world!');
        });
        test('relies upon CommonJS resolution', async (t) => {
            const r = await exec(`${CMD_ESM_LOADER_WITHOUT_PROJECT} ./esm-loader-entrypoint-cjs-fallback/relies-upon-cjs-resolution`);
            expect(r.err).toBe(null);
            expect(r.stdout.trim()).toBe('Hello world!');
        });
    ...
    test.suite('spawns child process', async (test) => {
        ...
        basic('ts-node w/tsconfig esm:true', () => exec(`${BIN_PATH} --esm ./esm-child-process/via-tsconfig/index.ts foo bar`));
        ...
        test.suite('esm child process and forking', (test) => {
            ...
            test('should be able to fork vanilla NodeJS script', macro, `${BIN_PATH} --esm --cwd ./esm-child-process/ ./process-forking-js/index.ts`);
            test('should be able to fork TypeScript script', macro, `${BIN_PATH} --esm --cwd ./esm-child-process/ ./process-forking-ts/index.ts`);
            test('should be able to fork TypeScript script by absolute path', macro, `${BIN_PATH} --esm --cwd ./esm-child-process/ ./process-forking-ts-abs/index.ts`);
        });
    });
